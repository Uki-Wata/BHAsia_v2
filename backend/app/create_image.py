...
def create_agent(input):
    ...
    agent = create_openai_functions_agent(ChatOpenAI(model="gpt-3.5-turbo",temperature=0), tools, prompt)
    agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)

    input_text = f"""
    Run the code below with python diagrams and save the output to the /Users/koki/Documents/BHAsia/backend/app/images path.
    {input}
    Please give the file name so that it is easy to understand.If the file exists, please overwrite it
    Return the file name for list.
    """
